...
func (t *Trie) Put( //nolint:gocognit // trie insertion with multiple node type handling
	key []byte,
	value []byte,
) error { //nolint:gocognit // trie insertion algorithm has inherent complexity
	...
	for {
		...
		if ext, ok := (*node).(*ExtensionNode); ok {
			...
			if matched < len(ext.Path) {
				...
				case matched == len(nibbles):
					branch.SetValue(value)
				default:
					return fmt.Errorf("trie put: too many matched (%v > %v)", matched, len(nibbles))
				}
		...
		return fmt.Errorf("trie put: unknown node type")
	}
}
