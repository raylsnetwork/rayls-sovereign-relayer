...
# Check if ../rayls-privacy-contracts exists and error if not
if [ ! -d "../rayls-privacy-contracts" ]; then
    RAYLS_CONTRACTS_DIR=$(resolve_path "$SCRIPT_DIR/../rayls-privacy-contracts")
    echo "Error: rayls-privacy-contracts directory not found at $RAYLS_CONTRACTS_DIR" >&2
    echo "       Try to git pull it over there." >&2
    exit 1
fi

# Check if ../governance-api exists and error if not
if [ "$GOVERNANCE_ENABLED" = "true" ] && [ ! -d "../rayls-privacy-pnh-governance-api" ]; then
    GOVERNANCE_DIR=$(resolve_path "$SCRIPT_DIR/../rayls-privacy-pnh-governance-api")
    echo "Error: rayls-privacy-pnh-governance-api directory not found at $GOVERNANCE_DIR" >&2
    echo "       Try to git pull it over there." >&2
    exit 1
fi

# Check if ../rayls-privacy-pnh-auditor-ui exists and error if not
if [ "$GOVERNANCE_API_ENABLED" = "true" ] && [ "$EXPLORERS_ENABLED" = "true" ] && [ ! -d "../rayls-privacy-pnh-auditor-ui" ]; then
    AUDITOR_UI_DIR=$(resolve_path "$SCRIPT_DIR/../rayls-privacy-pnh-auditor-ui")
    echo "Error: rayls-privacy-pnh-auditor-ui directory not found at $AUDITOR_UI_DIR" >&2
    echo "       Try to git pull it over there." >&2
    exit 1
fi
...
# Check if ../rayls-privacy-ops-api and ../rayls-privacy-custody-light exist when Ops API is enabled.
# Custody-light is a transitive dep of ops-api (HSM URL); both are bind-mounted at build time.
if [ "$OPS_API_ENABLED" = "true" ]; then
    if [ ! -d "../rayls-privacy-ops-api" ]; then
        OPS_API_DIR=$(resolve_path "$SCRIPT_DIR/../rayls-privacy-ops-api")
        echo "Error: rayls-privacy-ops-api directory not found at $OPS_API_DIR" >&2
        echo "       git clone it there or re-run with --no-ops-api." >&2
        exit 1
    fi
    if [ ! -d "../rayls-privacy-custody-light" ]; then
        CUSTODY_DIR=$(resolve_path "$SCRIPT_DIR/../rayls-privacy-custody-light")
        echo "Error: rayls-privacy-custody-light directory not found at $CUSTODY_DIR" >&2
...
if [ "$CLEAN_MODE" = "clean" ]; then
    cp "../rayls-privacy-contracts/.env.example-local" "../rayls-privacy-contracts/.env"
...
if [ "$GOVERNANCE_ENABLED" = "true" ] && ( [ ! -f "../rayls-privacy-pnh-governance-api/.env" ] || [ "$CLEAN_MODE" = "clean" ] ); then
    cp ../rayls-privacy-pnh-governance-api/config/.env.example ../rayls-privacy-pnh-governance-api/.env
fi

# Add governance database to DB_NAMES if governance is enabled
if [ "$GOVERNANCE_ENABLED" = "true" ]; then
    GOVERNANCE_DB=$(grep -E '^DATABASE_CONNECTIONSTRING=' "../rayls-privacy-pnh-governance-api/.env" | sed -n 's|.*://[^/]*/\([^?]*\).*|\1|p')
...
#      proofs-api:  ../rayls-privacy-gnark-api/Dockerfile     (USER appuser = 999:999)
...
#                                activate-business-roles-pn in rayls-privacy-contracts). PN_TOKEN_REGISTRY_ADMIN
                ...
                if [ -z "$addr" ] || [ "$addr" = "null" ]; then
                    ...
                    for role in $ADMIN_ROLES; do
                        ...
                        if ( cd ../rayls-privacy-contracts &&
                            ...
                            echo "    (cd ../rayls-privacy-contracts && npx hardhat grant-business-role --pn ${upper} --role ${role} --account ${addr} --delay 0)"
                        fi
                    done
                fi
                ;;
...
# PNTokenRegistryV1 (see activate-business-roles-pn in rayls-privacy-contracts), so PN_TOKEN_REGISTRY_ADMIN
    ...
    for participant in "${PARTICIPANTS[@]}"; do
        ...
        for role in $ADMIN_ROLES; do
            ...
            if ( cd ../rayls-privacy-contracts &&
                ...
                echo "    (cd ../rayls-privacy-contracts && npx hardhat grant-business-role --pn ${upper} --role ${role} --account ${operator_addr} --delay 0)"
            fi
        done
        for role in $USER_ROLES; do
            ...
            if ( cd ../rayls-privacy-contracts &&
                ...
                echo "    (cd ../rayls-privacy-contracts && npx hardhat grant-business-role --pn ${upper} --role ${role} --account ${user_addr} --delay 0)"
            fi
        done
...
clean_base_services() {
    ...
    # Sync gnark verifier contracts to rayls-privacy-contracts before building.
    # This ensures the on-chain verifiers match the gnark proving keys.
    GNARK_BUILD="../rayls-privacy-gnark-api/last_build"
    CONTRACTS_PAYMENTS="../rayls-privacy-contracts/src/rayls-protocol/Enygma/Enygma-Payments"
    CONTRACTS_DVP="../rayls-privacy-contracts/src/rayls-protocol/Enygma/Enygma-DVP"
    if [ -d "$GNARK_BUILD" ] && [ -d "$CONTRACTS_PAYMENTS" ]; then
        echo "Syncing gnark verifier contracts to rayls-privacy-contracts..."
...
# Generated by rayls-privacy-relayer-api/start_dev.sh — do not edit by hand.
...
networks:
  rayls-privacy-relayer-api_default:
    external: true
    name: rayls-privacy-relayer-api_default
...
AUTOMATION_PATH="../rayls-privacy-tests-automation"
...
if [ "$OPS_API_ENABLED" = "true" ] && [ -f "../rayls-privacy-contracts/.env" ]; then
    for participant in "${PARTICIPANTS[@]}"; do
        lc=$(echo "$participant" | tr '[:upper:]' '[:lower:]')
        upper=$(echo "$participant" | tr '[:lower:]' '[:upper:]')
        addr=$(grep -E "^PRIVACY_NODE_${upper}_DEPLOYMENT_PROXY_REGISTRY=" \
            "../rayls-privacy-contracts/.env" | cut -d= -f2)
...
if [ -f "../rayls-privacy-contracts/.env" ]; then
