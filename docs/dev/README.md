...
This solution will automatically initialize and update `.env` configurations, thus little action is required. In case something is not working as it should, you can safely delete `rayls-sovereign-contracts/.env` as well as any `rayls-sovereign-relayer/.<X>.env` or `rayls-sovereign-pnh-governance/config-dev.json` files and start again.
...
- A directory structure like this:
  ...
  parfin/                                <- this can be named something else
    rayls-sovereign-contracts/           <- required
    rayls-sovereign-relayer/             <- this repo
    axyl/                                <- required for local mode (see Axyl section below)
    rayls-sovereign-pnh-governance/      <- required for governance services. Use `--no-governance` to skip
    rayls-sovereign-gnark-api/           <- required for the proofs API
    rayls-sovereign-tests-automation/    <- optional; contracts are auto-synced here when present
  ```
- Use the `main` branch of each `rayls-sovereign-*` repository.

> **Note:** the full local stack also depends on components that are not open-sourced yet:
> the `axyl` node, and for the optional `--blockscout` / playground features the
> `rayls-privacy-blockscout` and `rayls-privacy-playground` repositories referenced below.
> Without access to those, the stack cannot be brought up end to end from the public
> repositories alone.
...
1. Your terminal's current working directory is at `rayls-sovereign-relayer`.
2. The script at `rayls-sovereign-relayer/start_dev.sh` is executable on your machine. You can run `chmod +x start_dev.sh` on your terminal to make it executable.

In the `rayls-sovereign-relayer` directory, type the following command on your terminal to run every service locally:
```
./start_dev.sh
```
...
## Running tests from rayls-sovereign-contracts
...
- `NEXT_PUBLIC_CHAIN_REGISTRY_ADDRESS` — extracted at start-up from `../rayls-sovereign-contracts/.env` (`PNH_DEPLOYMENT_PROXY_REGISTRY` for the Hub, `PRIVACY_NODE_{A,B}_DEPLOYMENT_PROXY_REGISTRY` for the PNs).
...
The `contracts` container generates Go bindings and copies them to your `rayls-sovereign-relayer` directory automatically, when you start the environment. You might need to start a clean environment so that it redeploys new contracts like so: `./start_dev.sh -c` or `./start_dev.sh --clean`
...
Or if you want more flexibility, open a terminal inside the container, where you'll get access to all the tools from the rayls-sovereign-contracts repository:
...
It is safe to delete `../rayls-sovereign-contracts/.env`, any `rayls-sovereign-relayer/.<X>.env` files and also `../rayls-sovereign-pnh-governance/config-dev.json` because they will be recreated and updated when you start the environment again with `./start_dev.sh --clean`.

## What should I do if I update any contracts or Privacy Node code?

After you update any contracts, hardhat tasks or scripts, tests, or even Privacy Node code, you will need to start a clean environment. That is because the rayls-sovereign-contracts repository is copied when you first started the environment, and it does not support live-reload. You can start a clean environment with `-c`/`--clean` flag like so:
